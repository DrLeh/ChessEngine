...
namespace ChessEngine.Tests
{
    ...
    {
        private void DoForEveryBoard(Action<IBoardRepresentation> action)
        {
            var type = typeof(IBoardRepresentation);
            var boardImplementations = Assembly.GetAssembly(type).GetTypes().Where(x => x.IsAssignableFrom(type) && !x.IsInterface);

            foreach (var board in boardImplementations.Select(x => Activator.CreateInstance(x)).Cast<IBoardRepresentation>())
            {
                action(board);
            }
        }
