...
{
    public class LinearSquareCentricBoard : IBoardRepresentation
    {
        public LinearSquareCentricBoard()
        {
        ...
        public Square[] LinearSquares { get; set; } = new Square[BoardContants.HeightAndWidth * BoardContants.HeightAndWidth];
        public Square[,] Squares
        {
            get
            {
                var multi = new Square[BoardContants.HeightAndWidth, BoardContants.HeightAndWidth];
                int counter = 0;
                int rank = 0;
                foreach (var square in LinearSquares)
                {
                    multi[counter, rank] = square;
                    counter++;
                    if (counter == 8)
                    {
                        counter = 0;
        ...
        public Square GetSquare(File file, int rank) => LinearSquares[(int)file - 1 + ((8 * rank - 1))];
        /// <summary>
        /// Rank is 1-8
        /// </summary>
        /// <param name="rank">1-8</param>
        ...
        {
            LinearSquares[(int)s.File - 1 + (8 * (s.Rank - 1))] = s;
        }
